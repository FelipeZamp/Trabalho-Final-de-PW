...
async def post_inserir(
    ...
    produto = Produto(nome=nome,valor=valor,desc=desc)
...
@router.get("/{id_produto:int}")
async def get_descricao(
    request: Request,
    id_produto: int = Path(),
    usuario: Usuario = Depends(obter_usuario_logado),
    ):
    produto = ProdutoRepo.obter_por_id(id_produto)
    return templates.TemplateResponse("produto/detalhe.html", {"request": request, "usuario": usuario, "produto": produto},)
